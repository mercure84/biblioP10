...
public class ReservationControllerREST {
    ...
    private void gererListeAttente(Livre livreParam) {
        ...
        if (absenceNewOption) {
            ...
            logger.info("[REST] Le Livre suivant doit être remis en stock : " + livreParam.getTitre() + " Id :" + livreParam.getId());
        }
    ...
    public Emprunt validerOption(@RequestHeader("Authorization") String token, int resaId) {
        ...
        logger.info("[REST] Nouvel emprunt : " + emprunt.getId() + emprunt.getLivre());
        logger.info("[REST] La réservation " + resa.getId() + " est terminée !");
        return emprunt;
